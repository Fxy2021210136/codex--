...
真实密钥、`.env`、`data/projects.json`、`data/auth.json`、`data/templates.json` 和 `data/ai-settings.json` 均不得提交到 GitHub。
...
- 自定义工序模板会通过 `/api/templates` 按用户保存到 `data/templates.json`，包含阶段、专业、工期、前置关系、Lag、资源投入、材料节点、空间展开维度等复用数据。
